...
<main class="flex-1 p-6 bg-gray-100 overflow-y-auto">
    <div class="flex items-center justify-between mb-4">
        <a class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:ring focus:ring-blue-300 transition-all duration-200" href="{{ url('Administrator/add-article-category') }}">Add {{$page_header}}</a>
    </div>
        ...
        <table class="w-full border-collapse border border-gray-300">
            <thead class="bg-gray-200">
                <tr>
                    ...
                    <th class="text-left px-4 py-2 border border-gray-300">Category Name</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="px-4 py-2 border border-gray-300">
                        <a class="px-2 py-1 bg-green-500 text-white rounded-md hover:bg-green-600 mr-2" href="{{ url('/Administrator/edit-article-category/' . $category['id']) }}">Edit</a>
                        <a  onclick="return confirm('Are you sure to processed?');" class="px-2 py-1 bg-red-500 text-white rounded-md hover:bg-red-600" href="{{ url('/Administrator/delete-article-category/' . $category['id']) }}">Delete</a>
                        <a onclick="return confirm('Are you sure to processed?');" class="px-2 py-1 bg-{{ $category['status'] == 'active' ? 'yellow' : 'blue' }}-500 text-white rounded-md hover:bg-{{ $category['status'] == 'active' ? 'yellow' : 'blue' }}-600" href="{{ $category['status'] == 'active' ? url('Administrator/article_category_inactive/'.$category['id']) : url('Administrator/article_category_active/'.$category['id']) }}">{{ $category['status'] == 'active' ? 'Inactivate' : 'Activate' }}
                        </a>
                    </td>
                </tr>
                @endforeach
